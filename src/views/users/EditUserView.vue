<!-- src/views/users/EditUserView.vue  ✅ EDIT USER PAGE (UPDATED) -->
<template>
    ...
    >
      ...
      <div>
        ...
        >
          ...
          <option v-for="lang in safeLanguages" :key="lang.code" :value="lang.code">
            {{ lang.name }}
          </option>
        </select>
      </div>
      ...
      <div>
        ...
        >
          ...
          <option v-for="r in safeRoles" :key="r.id" :value="Number(r.id)">
            {{ r.name }}
          </option>
        </select>
      </div>
      ...
      <div>
        ...
        >
          ...
          <option v-for="rg in safeRegions" :key="rg.id" :value="Number(rg.id)">
            {{ rg.name }}
          </option>
        </select>
      </div>
      ...
      <div v-if="isRetailStaff">
        ...
        >
          ...
          <option v-for="s in safeShops" :key="s.id" :value="Number(s.id)">
            {{ s.name }}
          </option>
        </select>
      </div>
...
function unwrapArray(payload) {
  // [] | {data: []} | {success:true, data: []} | {rows: []} | {items: []} | nested variants
...
function normalizeRole(r) {
  return { ...r, id: num(r?.id ?? r?.role_id), name: r?.name ?? r?.role_name ?? '' }
}
function normalizeRegion(rg) {
  return { ...rg, id: num(rg?.id ?? rg?.region_id), name: rg?.name ?? rg?.region_name ?? '' }
}
function normalizeShop(s) {
  return { ...s, id: num(s?.id ?? s?.shop_id), name: s?.name ?? s?.shop_name ?? '', shop_code: s?.shop_code ?? s?.code ?? '' }
}
function normalizeDealer(d) {
  return { ...d, id: num(d?.id ?? d?.dealer_id), name: d?.name ?? d?.dealer_name ?? '', dealer_code: d?.dealer_code ?? d?.code ?? '' }
}
function normalizeBrand(b) {
  return { ...b, id: num(b?.id ?? b?.brand_id), name: b?.name ?? b?.brand_name ?? '' }
}
function normalizeDivision(d) {
  return {
    ...
    brand_id: num(d?.brand_id ?? d?.brandId ?? d?.brandID)
  }
}
function parseCsvIds(value) {
  if (Array.isArray(value)) return value.map(num).filter(Boolean)
  if (typeof value === 'string') return value.split(',').map(v => num(v?.trim())).filter(Boolean)
  return []
}
// ----------------------------------
...
const filteredDivisions = computed(() => {
  ...
  if (!hasBrandLink) return safeDivisions.value
...
function friendlyMessageFromError(e) {
...
onMounted(async () => {
  ...
  try {
    const [rRoles, rRegions, rShops, rDealers, rLangs, rBrands, rDivs, userRes] =
      await Promise.all([
        api.get('/roles'),
        api.get('/regions'),
        api.get('/shops'),
    ...
    form.value = {
      ...
      shop_id: num(u.shop_id ?? u.retail_shop_id),
    ...
    console.error(e)
    error.value = friendlyMessageFromError(e)
  } finally {
    loading.value = false
  }
})
...
async function handleUpdate() {
  ...
  try {
    ...
    const payload = {
      ...
      shop_ids: Array.isArray(form.value.shop_ids)
        ? form.value.shop_ids.map(num).filter(Boolean)
        : []
    }

    // ✅ IMPORTANT: do NOT send password if blank
    const rawPwd = String(form.value.password || '').trim()
    if (rawPwd) {
      if (rawPwd.length < 6) {
        fieldErrors.value.password =
          'To change password, enter at least 6 characters. Leave it blank to keep the current password.'
        throw new Error(fieldErrors.value.password)
      }
    ...
    console.error(e)
    const msg = friendlyMessageFromError(e)

    // If backend returns the friendly password message, show it under the field too
    ...
    // keep your old UX (popup) if you want:
    // alert(msg)
  } finally {
    loading.value = false
  }
